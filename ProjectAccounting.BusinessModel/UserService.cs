...
namespace ProjectAccounting.BusinessModel
{
    public class UserService : IUserService
    {
        ...
        public UserService(ProjectAccountingContext projectAccountingContext)
        {
        ...
        public TblUser GetUserInfosByUserPass(string UserCode,string password)
        {
            try
            {
                //UserCode = "77777";
                //password = "123";
                ResponseModel response = new ResponseModel();
                var data = _projectAccountingContext.TblUsers.Where(x => x.UserCode == UserCode && x.Password == password).FirstOrDefault();
                //data = (TblUser)_projectAccountingContext.TblUsers.SingleOrDefault(x => x.Password == user.Password);
                if (data != null)
                {
                    return data;
                }
